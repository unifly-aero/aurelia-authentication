...
export let Saml = (_dec5 = inject(Storage, Popup, BaseConfig), _dec5(_class6 = class Saml {
  constructor(storage, popup, config) {
    this.storage = storage;
    this.config = config;
    this.popup = popup;
  ...
  open(options, userData) {
    ...
    const openPopup = this.config.platform === 'mobile' ? popup.eventListener(provider.redirectUri) : popup.pollPopup(qs => qs.error != null || qs.access_token != null);
...
export let Authentication = (_dec6 = inject(Storage, BaseConfig, OAuth1, OAuth2, AuthLock, Saml), _dec7 = deprecated({ message: 'Use baseConfig.loginRoute instead.' }), _dec8 = deprecated({ message: 'Use baseConfig.loginRedirect instead.' }), _dec9 = deprecated({ message: 'Use baseConfig.joinBase(baseConfig.loginUrl) instead.' }), _dec10 = deprecated({ message: 'Use baseConfig.joinBase(baseConfig.signupUrl) instead.' }), _dec11 = deprecated({ message: 'Use baseConfig.joinBase(baseConfig.profileUrl) instead.' }), _dec12 = deprecated({ message: 'Use .getAccessToken() instead.' }), _dec6(_class7 = (_class8 = class Authentication {
  constructor(storage, config, oAuth1, oAuth2, auth0Lock, saml) {
    this.storage = storage;
    this.config = config;
    this.oAuth1 = oAuth1;
    this.oAuth2 = oAuth2;
...
export let AuthService = (_dec13 = inject(Authentication, BaseConfig, BindingSignaler, EventAggregator), _dec14 = deprecated({ message: 'Use .getAccessToken() instead.' }), _dec13(_class9 = (_class10 = class AuthService {
